...
class _MainLayoutState extends State<MainLayout> {
  ...
  /// แปลงลิงก์ Google Drive เป็น direct link เพื่อให้แสดงรูปได้จริง
  String? get _profileImageUrl {
    final url = _userProfileImage;
    if (url.isEmpty || !url.startsWith('http')) return null;

    String? fileId;
    final byPath = RegExp(r'/d/([a-zA-Z0-9_-]+)').firstMatch(url);
    if (byPath != null) {
      fileId = byPath.group(1);
    } else {
      final byQuery = RegExp(r'id=([a-zA-Z0-9_-]+)').firstMatch(url);
      if (byQuery != null) fileId = byQuery.group(1);
    }

    if (fileId != null) return 'https://lh3.googleusercontent.com/d/$fileId';
    return url;
  }
                          ...
                          return Container(
                            ...
                            child: photo == null
                                ...
                                : Image.network(
                                    photo,
                                    fit: BoxFit.cover,
                                    // โหลดรูปไม่ได้ก็ถอยไปใช้อักษรย่อ
